...
theorem paperInferenceSoundEnv_all (R : LocalRely) (txnId : TxnId) (I : Assertion)
    (hStablePre : ∀ Fctxt, Logic.stableBiAssertion R (transformerPre I Fctxt))
    (hStablePost : ∀ Fctxt F, Logic.stableBiAssertion R (transformerPost Fctxt F)) :
    ∀ Fctxt env body F,
      inferPaperEffect txnId env body = some F →
      paperInferenceSoundEnv R txnId I Fctxt env body F := by
  sorry

theorem paperInferenceSound_all (R : LocalRely) (txnId : TxnId) (I : Assertion)
    (hStablePre : ∀ Fctxt, Logic.stableBiAssertion R (transformerPre I Fctxt))
    (hStablePost : ∀ Fctxt F, Logic.stableBiAssertion R (transformerPost Fctxt F)) :
    ∀ Fctxt body F,
      inferPaperEffect txnId ({ scalarVars := [], setVars := [] } : SymEnv) body = some F →
      paperInferenceSound R txnId I Fctxt body F := by
  intro Fctxt body
  induction body generalizing Fctxt with
  | skip =>
      intro F hInfer
      simp [inferPaperEffect] at hInfer
      subst F
      exact paperInferenceSound_skip R txnId I Fctxt (hStablePost Fctxt SetLanguage.empty)
  | letE x expr body ih =>
      intro F hInfer
      sorry
  | ite cond thenBranch elseBranch ihThen ihElse =>
      intro F hInfer
      cases hThen : inferPaperEffect txnId ({ scalarVars := [], setVars := [] } : SymEnv) thenBranch with
      | none =>
          simp [inferPaperEffect, hThen] at hInfer
      | some FThen =>
          cases hElse : inferPaperEffect txnId ({ scalarVars := [], setVars := [] } : SymEnv) elseBranch with
          | none =>
              simp [inferPaperEffect, hThen, hElse] at hInfer
          | some FElse =>
              simp [inferPaperEffect, hThen, hElse] at hInfer
              subst F
              exact paperInferenceSound_ite R txnId I Fctxt
                ({ scalarVars := [], setVars := [] } : SymEnv)
                cond thenBranch elseBranch FThen FElse
                (hStablePre Fctxt)
                (ihThen (Fctxt := Fctxt) (F := FThen) hThen)
                (ihElse (Fctxt := Fctxt) (F := FElse) hElse)
  ...
  | txn innerTxnId isolation body ih =>
      intro F hInfer
      simp [inferPaperEffect] at hInfer
  | txnRuntime innerTxnId isolation localDb snapshot body ih =>
      intro F hInfer
      simp [inferPaperEffect] at hInfer
