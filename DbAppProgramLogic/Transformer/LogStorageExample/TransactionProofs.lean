  ...
  | cons head tail ih =>
      ...
      · -- seq body: <body for head>; foreachRuntime continued
        ...
        · -- Body for head: subst doneVar (lit done) (subst entryVar (lit (record head)) <body>)
          ...
          · -- Log head: head's tableField = logTable
            -- Bridge head.visible (= headRow.visible) key info via existing helpers
            have hHeadTableLookup : head.lookup? tableField = some (.int logTable) := by
              rw [← hHeadVis]
              exact rowFieldInt?_eq_some_iff_lookup.mp
                (rowFieldInt?_tableField_of_key_wellFormed hWF hHeadRowMem hLogKey)
            have hHeadIdLookup : head.lookup? idField = some (.int n) := by
              rw [← hHeadVis]
              exact rowFieldInt?_eq_some_iff_lookup.mp
                (rowFieldInt?_idField_of_key hLogKey)
            -- Now eval the ITE's cond: it evaluates to some (.scalar (.bool true))
            -- because head.tableField = logTable (the cond's RHS).
            -- Apply localValid_ite_false.
            refine Logic.localValid_ite_false (selectTxnId q) _ _ _ _ _ ?_ ?_
            · -- True branch: insert (resultRecordExpr q n)
              intro _hEvalTrue
              -- Apply localValid_insert_false
              refine Logic.localValid_insert_false (selectTxnId q) _ _ _ ?_
              intro ld vd record hPre hEvalRecord _hFresh
              rcases hPre with ⟨hVd, hLdMem⟩
              subst hVd
              -- Eval the record: resultRecordExpr q (proj (lit head) idField) = record [(table, resultTable q), (id, n)]
              -- Which is resultRecord q n
              have hRecordEq : record = resultRecord q n := by
                simp only [resultRecordExpr, fieldExpr, Expr.subst, Expr.subst_lit,
                  Expr.substFieldExprs_cons, Expr.substFieldExprs_nil, Expr.eval,
                  Literal.toValue, Expr.evalFieldValues, Bind.bind, Option.bind,
                  Option.some_bind, Pure.pure, if_true, hHeadIdLookup,
                  Expr.int] at hEvalRecord
                -- hEvalRecord: some (Value.record ⟨[(table, int resultTable q), (id, int n)]⟩) = some (Value.record record)
                injection hEvalRecord with hEv
                injection hEv with hEv
                -- record's fields = expected fields
                rw [← hEv]
                rfl
              refine ⟨rfl, ?_⟩
              intro row
              -- ld' = ld ++ [Row.fromInsert (selectTxnId q) record] = ld ++ [resultRowLit q n]
              rw [hRecordEq]
              simp only [List.mem_append, List.mem_singleton]
              rw [hLdMem row]
              -- Show: row ∈ ld_mem ∨ row = Row.fromInsert (selectTxnId q) (resultRecord q n) ↔
              --       doneContrib q visibleDb (done ++ [head]) row
              unfold selectAllLogBody_doneContrib
              constructor
              · rintro (hOld | hNew)
                · -- Old contribution from done
                  rcases hOld with ⟨rec, hRecMem, srcRow, hSrcMem, hSrcVis, hOut⟩
                  refine ⟨rec, ?_, srcRow, hSrcMem, hSrcVis, hOut⟩
                  exact List.mem_append_left _ hRecMem
                · -- New: row from head's log contribution
                  refine ⟨head, ?_, headRow, hHeadRowMem, hHeadVis, ?_⟩
                  · exact List.mem_append_right _ (List.mem_singleton.mpr rfl)
                  · -- denote(selectEntryResultEffect q headRow) at visibleDb row
                    -- = selectLogResultEffect q headRow ∨ selectArchiveResultEffect q headRow
                    -- For log headRow, selectLogResultEffect at row matches if row = resultRowLit q n
                    unfold selectEntryResultEffect
                    rw [SetLanguage.denote_union]
                    left
                    -- selectLogResultEffect q headRow at row = ∃ m, (∃ id, headRow.key? = some (logTable, id))
                    --   ∧ rowFieldInt? headRow idField = some m ∧ row = resultRowLit q m
                    refine ⟨n, ⟨n, hLogKey⟩, ?_, hNew⟩
                    exact rowFieldInt?_idField_of_key hLogKey
              · rintro ⟨rec, hRecMem, srcRow, hSrcMem, hSrcVis, hOut⟩
                rcases List.mem_append.mp hRecMem with hInDone | hIsHead
                · left
                  exact ⟨rec, hInDone, srcRow, hSrcMem, hSrcVis, hOut⟩
                · right
                  -- rec = head, srcRow corresponds to head
                  have hRecEq : rec = head := List.mem_singleton.mp hIsHead
                  subst hRecEq
                  -- denote(selectEntryResultEffect q srcRow) at visibleDb row
                  unfold selectEntryResultEffect at hOut
                  rw [SetLanguage.denote_union] at hOut
                  rcases hOut with hLog | hArch
                  · -- selectLogResultEffect: row = resultRowLit q m for some m where srcRow.key has logTable id m
                    rcases hLog with ⟨m, ⟨id, hKey⟩, _hN, hRow⟩
                    -- srcRow.key? = some (logTable, id). srcRow.visible = head = headRow.visible.
                    -- Under wellFormedness, headRow.key? = some (logTable, n). Need id = n, m = n.
                    have hKeyEq : srcRow.key? = headRow.key? := by
                      unfold Row.key?
                      rw [hSrcVis, ← hHeadVis]
                    rw [hLogKey] at hKeyEq
                    rw [hKeyEq] at hKey
                    have : id = n := (Prod.mk.inj (Option.some.inj hKey)).2
                    subst this
                    have hMEq : m = n := by
                      have hExtract := rowFieldInt?_idField_of_key hLogKey
                      -- need to relate _hN to this
                      have hKeyEqFromHead : srcRow.key? = some (logTable, n) := by
                        rw [hKeyEq, hLogKey]
                      have hExtractSrc := rowFieldInt?_idField_of_key hKeyEqFromHead
                      rw [hExtractSrc] at _hN
                      exact (Option.some.inj _hN).symm
                    subst hMEq
                    rw [hRow]
                    rfl
                  · -- selectArchiveResultEffect: requires srcRow.key = some (archiveTable, _).
                    -- But srcRow.key? = headRow.key? = some (logTable, n) ≠ archive. Contradiction.
                    exfalso
                    rcases hArch with ⟨_, _, _, ⟨_id, hAKey⟩, _, _, _, _, _⟩
                    have hKeyEq : srcRow.key? = headRow.key? := by
                      unfold Row.key?
                      rw [hSrcVis, ← hHeadVis]
                    rw [hLogKey] at hKeyEq
                    rw [hKeyEq] at hAKey
                    have : archiveTable = logTable := (Prod.mk.inj (Option.some.inj hAKey)).1
                    exact logTable_ne_archiveTable this.symm
            · -- False branch: should not happen for log head
              intro hEvalFalse
              exfalso
              -- The cond's eval should be true for log head
              simp [Expr.subst, Expr.eval, Literal.toValue, isLogExpr, isTableExpr, eqExpr,
                fieldExpr, Expr.int, tableField, hHeadTableLookup] at hEvalFalse
