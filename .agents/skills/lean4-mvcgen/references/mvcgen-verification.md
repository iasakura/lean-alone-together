...
The `mvcgen` tactic implements a _monadic verification condition generator_:
It breaks down a goal involving a program written using Lean's imperative `do` notation into a number of pure _verification conditions_ that discharge said goal.
...
After specifying the loop invariant, the proof can be shortened to just `all_goals mleave; grind`.
Note that the case labels have a unique prefix `vc1`, `vc2`, `vc3` etc.
...
* `vc2.a.pre1` is meant to prove that the hypotheses of a goal implies the precondition of a specification (of `forIn`).
* `vc3.a.post` is meant to prove that the postcondition of a specification (of `forIn`) implies the target of the goal.

It is helpful to compare this to a traditional correctness proof:
...
This proof is similarly succinct as the proof using `mvcgen`.
...
* `do` blocks using control flow constructs such as (nested) `for` loops with early `return`.
...
The example function `nodup` below decides this predicate:
...
theorem mkFreshN_correct (n : Nat) : ((mkFreshN n).run' s).Nodup := by
  ...
  -- The `mkFreshN` and `mkFresh` arguments to `mvcgen` add to an internal
  -- `simp` set and makes `mvcgen` unfold these definitions.
  ...
  -- Note that the invariant may refer to the state through function argument `state : Supply`.
  -- Since the next number to accumulate is the counter, it is distinct to all accumulated numbers.
...
These can either be passed as arguments to `mvcgen` or registered in a global (or `scoped`, or `local`) database of specifications using the `@[spec]` lemma:
...
theorem mkFreshN_spec (n : Nat) : ⦃⌜True⌝⦄ mkFreshN n ⦃⇓ r => ⌜r.Nodup⌝⦄ := by
  -- `mvcgen [mkFreshN, mkFresh_spec]` if `mkFresh_spec` were not registered with `@[spec]`
  mvcgen [mkFreshN]
  -- As before:
  case inv1 => exact ⇓⟨xs, acc⟩ state => ⌜(∀ x ∈ acc, x < state.counter) ∧ acc.toList.Nodup⌝
  all_goals mleave; grind
...
theorem mkFreshN_correct (n : Nat) : ((mkFreshN n).run' s).Nodup :=
  -- The type `⦃⌜True⌝⦄ mkFreshN n ⦃⇓ r => ⌜r.Nodup⌝⦄` of `mkFreshN` is definitionally equal to
  -- ∀ (n : Nat) (s : Supply), True → ((mkFreshN n).run' s).Nodup
  mkFreshN_spec n s True.intro
```

Here, the notation `⦃fun state => ⌜state.counter = c⌝⦄ mkFresh ⦃⇓ r state => ⌜r = c ∧ c < state.counter⌝⦄` denotes a _Hoare triple_ (cf. `Std.Do.Triple`).
Specifications for monadic functions always have such a Hoare triple result type.

### Hoare triples

A Hoare triple `⦃P⦄ prog ⦃Q⦄` means "If the _precondition_ `P` on the state holds, then the _postcondition_ `Q` holds after running the monadic _program_ `prog`".
So the specification theorem for `mkFresh` says:

> If `c` refers to the `counter` field of the `Supply` prestate, then running `mkFresh` returns `c` and modifies the `counter` of the poststate to be larger than `c`.
...
Hoare triples are defined in terms of a logic of stateful predicates plus a weakest precondition semantics `wp⟦·⟧` that translates monadic programs into this logic:
...
The `WP` type class maps a monad `m` to its `ps : PostShape`, and this `PostShape` governs the exact
shape of the `Triple`.
Many of the standard monad transformers such as `StateT`, `ReaderT` and `ExceptT` come with a
canonical `WP` instance.
...
For `StateM` programs, the following type is definitionally equivalent to `Triple`:
...
`PostCond ps`; in case of `StateM` by adjoining it with an empty tuple `PUnit.unit`.
The shape of postconditions becomes more interesting once exceptions enter the picture.
The notation `⌜p⌝` embeds a pure hypotheses `p : Prop` into a stateful assertion.
Vice versa, any stateful hypothesis `h : Assertion ps` is called _pure_ if it is equivalent to `⌜p⌝`
for some `p : Prop`.
...
theorem add_spec_pre (x y : UInt8) :
    ⦃⌜x.toNat + y.toNat ≤ UInt8.size⌝⦄ x +? y ⦃⇓ r => ⌜r.toNat = x.toNat + y.toNat⌝⦄ := ...
```
...
This is referred to as _framing_ hypotheses (cf. the `mpure` and `mframe` tactics).
...
Suppose that `mkFresh` is generalized to work in any base monad `m` under `StateT Supply`.
...
The `WPMonad` type class asserts that `wp⟦·⟧` distributes over the `Monad` operations ("monad morphism").
...
Exceptions are the reason why the type of postconditions `PostCond α ps` is not simply a single condition `α → Assertion ps` for the success case.
To see why, suppose the latter was the case, and suppose that program `prog` throws an exception in aprestate satisfying `P`.
Should we be able to prove `⦃P⦄ prog ⦃⇓ r => Q r⦄`?
Certainly, `Q` does not hold because there is no result `r` it could hold for!

So there are two reasonable options, inspired by nontermination in traditional program logics:

1. _Total_ correctness interpretation: `⦃P⦄ prog ⦃⇓ r => Q r⦄` asserts that, given `P` holds, then `prog` terminates _and_ `Q` holds for the result.
2. _Partial_ correctness interpretation: `⦃P⦄ prog ⦃⇓ r => Q r⦄` asserts that, given `P` holds, and _if_ `prog` terminates _then_ `Q` holds for the result.
...
Just as any `StateT σ`-like layer in the monad stack gives rise to a `.arg σ` layer in the `PostShape` `ps`
that `WP` maps into, any `ExceptT ε`-like layer gives rise to a `.except ε` layer in the `PostShape`.

Every `.arg σ` layer adds another `σ → ...` layer to the language of `Assertion`s.
Every `.except ε` layer leaves the `Assertion` language unchanged, but adds another exception
...
Hence the `WP` instance for `EStateM ε σ` maps to the `PostShape` `.except ε (.arg σ .pure)`, just
...
The `WP` instance defines the weakest precondition interpretation of a monad `m` into a predicate transformer `PredTrans ps`,
...
The `WP` instance above translates programs in `Result α` to predicate transformers in `PredTrans ps α`.
That is, a function in `PostCond α ps → Assertion ps`, mapping a postcondition to its weakest precondition.
...
In this case, verification conditions of the form `H ⊢ₛ T`, for `H P : Std.Do.SPred σs` will be left behind.
The assertion language `Assertion ps` translates into such an `Std.Do.SPred σs` as follows:
...
A common case for when a VC of the form `H ⊢ₛ T` is left behind is when the base monad `m` is polymorphic.
In this case, the proof will depend on a `WP m ps` instance which governs the translation into the `Assertion` language, but the exact correspondence to `σs : List (Type u)` is yet unknown.
...
The Lean 4 test file [`tests/lean/run/spredProofMode.lean`](https://github.com/leanprover/lean4/blob/76971a88fff3b3df75dceb588b5bd98b1552bafc/tests/lean/run/spredProofMode.lean) contains many examples of that proof mode to learn from, and the reference manual has a list of all proof mode tactics.
...
* Be sure to read the documentation comments for `Std.Do.SPred`, `Std.Do.PostCond`, `Std.Do.PredTrans`, `Std.Do.Triple` and the tactics `mvcgen`, `mspec`, `mintro`, etc.
